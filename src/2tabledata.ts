...
import {
  getElementCdsPersistenceName,
  getArtifactCdsPersistenceName,
} from '@sap/cds-compiler';
...
module.exports = async (model, options = {}) => {
  ...
  const dirs = Array.isArray(options.dirs)
    ? options.dirs
    : _csvDirs(model.$sources.map(path.dirname));
      ...
      dirs.map(async (dir) => {
        ...
        return Promise.all(
          files
            .filter(_csvs)
            .map((file) =>
              _tabledata4(dir, file, cds.linked(model), baseDir, naming),
            ),
        );
      }),
    )
...
async function _tabledata4(dir, csvFile, model, baseDir, naming) {
  ...
  if (!entity) {
    let message = `no entity '${entityName}' found for CSV file '${path.relative(
      process.cwd(),
  ...
  if (entity.elements.locale) {
    ...
    if (locale) {
      const localeKey = getElementCdsPersistenceName(
        entity.elements.locale.name /*usually 'LOCALE'*/,
        naming,
      );
...
function _entity4(name, csn) {
  ...
  if (entity['@cds.persistence.skip'] === true)
    return logger.warn(`[hdbtabledata] exclude skipped entity '${name}'`);
...
function _csvDirs(sources) {
  ...
  for (const src of sources) {
    for (const data of ['/data', '/csv']) {
      for (const each of [
        src + data,
        src + '/src' + data,
        src + '/..' + data,
      ]) {
        const folder = path.resolve(cds.root, each);
...
function _csvs(filename, _, allFiles) {
  ...
  if (/(.*)[._]texts\.csv$/.test(filename)) {
    ...
    const monoLangFiles = allFiles.filter((file) =>
      new RegExp(basename + '_texts_').test(file),
    );
    if (monoLangFiles.length > 0) {
      logger._debug &&
        logger.debug(
          `[hdbtabledata] ignoring '${filename}' in favor of [${monoLangFiles}]`,
        );
      return false;
    }
  }
